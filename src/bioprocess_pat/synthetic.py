...
def generate_fed_batch(
    n_hours: int = 168, seed: int = 42, deviation_start: int | None = None
) -> pd.DataFrame:
    if n_hours < 24:
        raise ValueError("n_hours must be >= 24")
    ...
    feed_rate = np.clip((t - 24) / 80, 0, 1.2) * (1 - 0.25 * late)
    feed_rate += rng.normal(0, 0.015, n_hours)
    ...
    if deviation_start is not None:
        if not 0 <= deviation_start < n_hours:
            raise ValueError("deviation_start must be within run")
    ...
    productivity = 0.45 * biomass + 0.6 * feed_rate - 0.25 * lactate
    productivity += rng.normal(0, 0.15, n_hours)
    quality = 1 - 1.6 * np.abs(ph - 7.0) - 0.8 * np.abs(temperature - 37.0)
    quality -= 0.12 * lactate
    quality += rng.normal(0, 0.03, n_hours)
    return pd.DataFrame(
        {
            "hour": t.astype(int),
